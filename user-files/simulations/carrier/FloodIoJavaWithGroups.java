...
public class FloodIoJavaWithGroups extends Simulation {
    ...
    ScenarioBuilder flood_io = scenario("flood_io")
            .during(duration).on(
                    group("first_group").on(
                                    exec(Step1GET).exec(Step1POST)
                            )
                            .group("second_group").on(
                                    exec(Step2GET).exec(Step2POST)
                            )
                            .exec(Step3GET)
                            .exec(Step3POST)
                            .exec(Step4GET)
